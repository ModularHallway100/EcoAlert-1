...
const simulateWave = (min: number, max: number): number => {
    const now = Date.now();
...
export function useEnvironmentalData() {
  ...
  useEffect(() => {
    const simulateData = () => {
        const currentAqi = simulateWave(20, 450);
        const currentPh = simulateWave(5.5, 8.5);
        const currentTurbidity = simulateWave(1, 100);
        const currentNoise = simulateWave(30, 120);
